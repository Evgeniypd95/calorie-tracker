...
export default function ConversationalOnboardingScreen({ navigation, route }) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (response.calculatedPlan) {
        ...
        // ONLY show finish button if ALL required fields are present
        const hasAllFields = response.extractedData.name &&
                            response.extractedData.age &&
                            response.extractedData.weight &&
                            response.extractedData.height &&
                            response.extractedData.gender &&
                            response.extractedData.goal &&
                            response.extractedData.activityLevel;

        setShowFinishButton(hasAllFields);
      }
  ...
  const handleFinish = () => {
    // Pass data to signup screen
    navigation.navigate('Signup', {
      onboardingData: {
        ...extractedData,
        dailyCalorieTarget: calculatedPlan.dailyCalories,
        proteinTarget: calculatedPlan.protein,
        carbsTarget: calculatedPlan.carbs,
        fatTarget: calculatedPlan.fat
      }
    });
  };
